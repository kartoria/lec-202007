...
<body>
	...
	<form id="form" action="<%=request.getContextPath()%>/BuyCartServlet"
		method="post">
		<input type="hidden" value="<%=memId %>" name="memId"> <input
			...
			<div class="container">
				...
				<div class="shopping-cart">
					<div class="column-labels">
						<label class="product-image">이미지</label> <label
							class="product-details">상품명</label> <label class="product-price">가격</label>
						<label class="product-quantity">구매</label> <label
							class="product-removal">삭제</label>
					</div>
					<% String chkPoint;
				...
				chkPoint = "chkPoint"+i;%>
					<div class="product">
						...
						<div class="product-price"><%=boardList.get(i).getBoardPrice() %></div>
						<div class="product-quantity">
						
							<input type="hidden" value="<%=boardList.get(i).getBoardNo() %>"
								class="chkPoint" name=chkPoint id="<%=chkPoint%>">
							<%
					int price = Integer.parseInt(boardList.get(i).getBoardPrice());
						if(point>price) {%>
						 
						 <input type="button" class="remove-product" onclick="buy(<%=i %>)" value="Buy">
							<%} else{%>
								<span> 구매불가</span>
							<%} %> 
						</div>
						<div class="product-removal">
							<input type="button" class="remove-product"
								onclick="remove(<%=i %>)" value="Remove">
					...
					<div class="totals">
						<div class="totals-item">
							<label>보유 포인트 :</label>
							<div class="totals-value" id="cart-subtotal"><%=point %>P</div>
						</div>
					</div>
				</div>
			</div>
