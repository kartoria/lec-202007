		...
		<div class="container">
			<div class="row">
				<table>
					...
					<tbody>
						<tr id = "inputTr">
							<td><a href="noticeView?noticeNo=<%=noticeList.get(i).getNoticeNo()%>"><%=noticeList.get(i).getNoticeTitle()%></a></td>
							<td>관리자</td>
				...
				<div style="text-align: center">
					...
					%>
									<a href="<%=request.getContextPath()%>/noticeMain?pageNo=<%=pagingVO.getFirstPageNo() - pagingVO.getPageCount()%>">[이전]</a>
					<%
								}
					%>
					<%
					...
					%>
									<a href="<%=request.getContextPath()%>/noticeMain?pageNo=<%=pNo%>">[<%=pNo%>] </a>
					<%
								}
								if (pagingVO.getLastPageNo() < pagingVO.getTotalPageCount()) {
					%>
									<a href="<%=request.getContextPath()%>/noticeMain?pageNo=<%=pagingVO.getFirstPageNo() + pagingVO.getPageCount()%>">[다음]</a>
					<%
								}
							}
						}else{ %>
				...
				<button type="button" id="insertNoticeBtn"
						...
						onclick="location.href='<%=request.getContextPath()%>/insertNotice'">게시글 등록</button>
			</div>
		</div>
	</div>
