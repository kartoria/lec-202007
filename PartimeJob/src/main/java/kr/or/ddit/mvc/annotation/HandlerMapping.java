...
public class HandlerMapping implements IHandlerMapping {
		...
		if(basePackages!=null && basePackages.length>0) {
			...
			try {
				while (it.hasNext()) {
					...
					Map<Method, Annotation> methods = ReflectionUtils.getMethodsWithAnnotationAtClass(controllerClass, RequestMapping.class, 
													String.class);
					for( Entry<Method, Annotation> entry : methods.entrySet() ) {
						...
						RequestMappingCondition mappingCondition = 
								new RequestMappingCondition(uri, requestMethod);
	...
	public RequestMappingInfo findCommandHandler(HttpServletRequest req) {
		...
		if(mappingInfo!=null)
			logger.info("url : {}, handler :  {}", url, 
						mappingInfo.getCommandHandler().getClass().getSimpleName()
						+"."+mappingInfo.getHandlerMethod().getName());
		else logger.error("url : {}에 대한 핸들러가 없음.", url);
		return  mappingInfo;
	}
