...
<div class="container-fluid">
	<div class="row">
		<div class="col-12">
			<h5 class="ml-2"><i class="fas fa-home"></i> 내 강의실 > 공지사항</h5>
		</div>
	</div>
	...
	<div class="row">
		<div class="col-11">
			<h5 class="pb-5 ml-2">강의 관련 공지 사항을 조회할 수 있습니다.</h5>
		</div>
			...
			<security:authorize access="hasRole('ROLE_PROFESSOR')">
				...
				<c:set var="authMember" value="${principal.realMember }" />
					<a class="btn waves-effect waves-light btn-info px-4 py-2"
						href="${cPath}/myclass/${lecCode}/notiInsertForm.do" role="button"
						id="insertBtn">등록</a>
			</security:authorize>
		</div>
	</div>
			...
			<table class="table">
				...
				<tbody>
					...
					<c:if test="${not empty boardList }">
						<c:forEach items="${boardList }" var="board" varStatus="status">
							<tr>
								...
								<c:url var="viewURL" value="/myclass/${lecCode}/${board.boNo}/notiView.do" />
					...
					<c:if test="${empty boardList }">
						<tr>
							<td colspan="5" align="center">조건에 맞는 게시글이 존재하지 않습니다.</td>
						</tr>
					</c:if>
				</tbody>
			</table>
	...
	<div class="row">
		<div class="col-1"></div>
		<div class="dataTables_paginate paging_simple_numbers col-8" id="pagingArea">
			...
			<div id="searchArea" class="row p-0 justify-content-right">
				...
				<div class="col-3 p-0 text-right">
					<select name="searchType" class="form-control h-auto" id="selectSearch">
						...
						<option value="title" ${'date' eq param.searchType?"selected":"" }>날짜</option>
					</select>
				</div>
